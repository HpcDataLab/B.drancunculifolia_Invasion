...
#IMAC path 
setwd("/Volumes/DATALacie/OneDrive - UNAMII/0. UNAM - Juriquilla/4. COLABORACIONES/6. Geraldo/3. Data/GBIF")
...
write.csv(uniqueLoc,"BaccarisLocUnique.csv")
...
library(tiff)
tiff(file = "heatmap", units="in", width=9, height=9, res=600)
...
bg <- dismo::randomPoints(bioVars[[1]], n = nrow(uniqueLoc))
...
test <- which(random$occs.grp == 1)                                                # 25%
train <- which(random$occs.grp == 2 | random$occs.grp == 3 | random$occs.grp == 4) # 75%

testCSV <- uniqueLoc[test,]
trainCSV <- uniqueLoc[train,]
